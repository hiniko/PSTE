...
Real documents, written by people, under licences that permit redistribution and
modification. The eval rewrites these. No tool creates a document here, and no
document here changes.

## What is here

Fourteen documents in two classes: nine that people wrote, and five that a model
wrote. None is share-alike, so every rewrite of them falls under the licence of this
project.
...
| `migration-django-upgrade` | migration guide | BSD-3-Clause |
...
The skill has two jobs. It rewrites a document that a person wrote, and it
constrains what a model writes. A corpus of human documentation tests the first job
...
version changing, and two baselines from different models are not comparable.
...
An answer generated on a configured machine is not a baseline. `claude -p` reads
the settings of the person who runs it.

One such subprocess, asked what it held, reported a terse output style from a
plugin, a global CLAUDE.md, three session hook injections, and more than fifty
skills. Each of those changes how the model writes, and a terse style is part of the
effect this project claims for the skill.

Command line flags switch most of that off, but a flag list is a deny list. A new
configuration surface appears, and the baseline is quietly contaminated again.
...
**It is the responsibility of whoever runs this to keep the environment clean.** The
tool does not inspect the machine, and it does not ask the model to describe its own
context. Both are weaker than a container that never held the configuration.

Every one of them fails the checker at level 2. That is the point: a corpus of
documents that already passed would measure nothing.

**The markup stays.** A document from a documentation site carries front matter,
template calls, and links. A rewrite has to survive them, so removing them would hide
the failure rather than test for it.
...
that several people wrote over several years in different voices.
...
An earlier version of the eval gave a model a one-line brief and asked each arm to
**write** a document. That measured nothing. Each arm invented different content, so
content and form moved together, and a reader cannot separate the two.

For one brief, the three arms produced 310, 239, and 741 words of different
...
the bytes it received, and appends the entry:
...
types those by hand gets one wrong eventually, and the corpus check then fails for a
...
1. **The licence must permit redistribution and modification.** Allowed: `CC0-1.0`,
   `public-domain`, `MIT`, `Apache-2.0`, `BSD-2-Clause`, `BSD-3-Clause`,
   `CC-BY-4.0`, `CC-BY-SA-4.0`, `CC-BY-SA-3.0`. A rewrite is a modification, and
   this repository is public, so a no-derivatives or non-commercial licence does
   not permit a rewrite at all.
...
2. **Record the provenance:** the URL, the author or project, the licence, and the
   date of retrieval.
...
5. **Attribution travels with the result.** `corpus.attribution()` builds the notice,
   ...
   the notice must say the text is a modification.
...
(`skill/hooks/pste-config.js`): runbook, release notes, incident report, error
message, documentation, api docs, migration guide, and so on.

The corpus covers the types that trigger strict mode in real use, because the skill
